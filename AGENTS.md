...
- Release payload commit, final GitHub Actions run, and Git-marketplace installation evidence are pending the final push. Initial run `33189714952` exposed Ubuntu SQLite CLI, setup-node executable-path, and x64-loader portability defects. Second run `33191248912` then exposed two deeper problems: a file-level loader bind leaked Bubblewrap mount descriptors on Ubuntu, and `{dev, ino}` checkpoint receipts could accept a byte-identical replacement when the filesystem immediately reused the inode. The sandbox now mounts read-only `/usr/lib` and `/usr/lib64` roots behind ABI aliases without the descriptor leak. `checkpoint.v2` binds device, inode, creation time, and change time after closing the writable publication descriptor; it rejects old receipts, pathname replacements, and later hard-link events.
...
- The two deterministic facade bundles have composite SHA-256 `0241cda093ecbf7f7949dc876bdb21df0438781854c183e1740e17da19784a3b`: `server.mjs` is `3d7655bd3032e9fc7475a25b0f8b4ed836bda7640fee4bc08e84b63e92b66cfe` and `upstream-supervisor.mjs` is `9e1391d3bdde10db620daadbb2529194e6aa7e9043b37fea09e688b1eaa7739a`. The reviewed facade source projection is `3ff6e5fc0a08ac4d31305c979779cd4238cfd36dce5c80511eabfffe5846ae61`.
