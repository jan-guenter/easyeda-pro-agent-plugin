...
import type {
  SchematicComponentInspectionItem,
  SchematicComponentInspectionOperations,
...
function bomGroupKey(component: SchematicComponentInspectionItem, groupBy: string): string {
  if (groupBy === 'lcsc') {
    return component.lcsc.length > 0 ? component.lcsc : component.value;
  }
  if (groupBy === 'footprint') {
    return component.footprint.length > 0 ? component.footprint : 'no-footprint';
  }
  return component.value.length > 0 ? component.value : 'no-value';
}

function createBomGroup(component: SchematicComponentInspectionItem) {
  return {
...
async function generateBom(
  ...
  const result = await dependencies.schematicComponentInspection.listComponents();
  const groupBy = typeof params.groupBy === 'string' ? params.groupBy : 'value';
...
export function createReadOnlyOperations(
  ...
  return {
    ...
    async getNetDetail(params) {
      return dependencies.getNetDetail(
        requiredString(params.netName, 'netName'),
    ...
    async getPrimitiveSnapshot(params) {
      return dependencies.schematicTransactionOperations.getPrimitiveSnapshot(
        requiredString(params.primitiveId, 'primitiveId'),
    ...
    async getPinNoConnect(params) {
      return dependencies.getPinNoConnect(
        requiredString(params.primitiveId, 'primitiveId'),
        requiredString(params.pinNumber, 'pinNumber'),
      );
    },
    ...
    async getDeviceByLcscId(params) {
      const lcscId =
        typeof params.lcscId === 'string' || typeof params.lcscId === 'number'
          ? String(params.lcscId)
          : '';
