// Every EasyEDA Pro API interaction lives behind createDispatcher(toolkit).
// The authenticated loader bakes this module into
// The imported extension; there is no runtime dispatcher replacement path.
// Host runtime globals are resolved through DispatcherToolkit (see toolkit.ts).
...
import { createDispatcherDomainRouter } from './dispatcher-domain-router.js';
import type { DispatcherDomainRouter } from './dispatcher-domain-router.js';
import { createBoardInspectionOperations } from './board-inspection.js';
...
import { createDesignRuleCheckOperations } from './design-rule-check-operations.js';
import type { DesignRuleCheckOperations } from './design-rule-check-operations.js';
import { createExportOperations } from './export-operations.js';
import type { ExportOperations } from './export-operations.js';
import { createPcbMutationOperations } from './pcb-mutation-operations.js';
import type { PcbMutationOperations } from './pcb-mutation-operations.js';
import { createPcbReadOperations } from './pcb-read-operations.js';
import type { PcbReadOperations } from './pcb-read-operations.js';
import { createPcbWriteOperations } from './pcb-write-operations.js';
...
import { createReadOnlyOperations } from './read-only-operations.js';
import type { ReadOnlyOperations } from './read-only-operations.js';
import { createSystemApiOperations } from './system-api-operations.js';
import type { SystemApiOperations } from './system-api-operations.js';
import { createSchematicComponentInspectionOperations } from './schematic-component-inspection.js';
import type { SchematicComponentInspectionOperations } from './schematic-component-inspection.js';
import { createSchematicInspectionOperations } from './schematic-inspection.js';
import type { SchematicInspectionOperations } from './schematic-inspection.js';
import { createSchematicTransactionOperations } from './schematic-transaction-operations.js';
import type { PublicTextAlignMode, SchematicTransactionOperations } from './schematic-transaction-operations.js';
import type { Dispatcher, DispatcherToolkit } from './toolkit.js';
import { isRecord, log, logRecoverableError } from './utils.js';
...
// The toolkit for the baked dispatcher instance. Set by createDispatcher().
...
function normalizeWireLine(line: unknown): { x: number; y: number }[] {
...
function pointOnPolyline(
  ...
  if (points.length === 1) {
    ...
    return onlyPoint ? samePoint(point, onlyPoint) : false;
  }
  for (let i = 0; i + 1 < points.length; i += 1) {
    const start = points[i];
    const end = points[i + 1];
    if (start && end && pointOnAxisAlignedSegment(point, start, end)) {return true;}
  }
  return false;
}
...
function polylineIntersection(
  ...
  for (let i = 0; i + 1 < a.length; i += 1) {
    for (let j = 0; j + 1 < b.length; j += 1) {
      ...
      const intersection = axisAlignedSegmentIntersection(a1, a2, b1, b2);
...
async function collectPinCoordinateNets(): Promise<Map<string, string>> {
  ...
  try {
    ...
    if (!message.includes('SCH_PrimitiveComponent class not found')) {
      logRecoverableError('failed to build pin coordinate map for net-collision check', error);
    }
  }
  return coordToNet;
}
...
async function collectFlagPortCoordinateNets(): Promise<Map<string, string>> {
  ...
  try {
    ...
    logRecoverableError('failed to read net flags/ports for net-collision check', error);
  }
  return coordToNet;
}
  ...
  if (schWireClass && typeof schWireClass.getAll === 'function') {
    try {
      ...
      for (const wire of wires) {
        const wireNet = nativeScalarString(safeGetState(wire, 'Net'));
      ...
      logRecoverableError('failed to read existing wires for duplicate-net-label suppression', error);
    }
  }
...
async function findForeignNetCollision(
  ...
  if (schWireClass && typeof schWireClass.getAll === 'function') {
    ...
    try {
      ...
      logRecoverableError('failed to read existing wires for net-collision check', error);
      wires = [];
    }
  ...
  for (const [key, foreignNet] of foreignMap) {
    ...
    if (!p) {continue;}
    if (foreignNet && foreignNet !== netName && pointOnPolyline(p, points)) {
...
interface SchematicPoint { x: number; y: number }
interface SchematicNetNode { component: string; pin: string; x?: number; y?: number; source?: string }
interface SchematicNetEntry { netName: string; nodes: SchematicNetNode[] }
...
const NET_DETAIL_STAGE_TIMEOUT_MS = 5000;
...
function readComponentNet(component: unknown): string {
  ...
  if (isRecord(otherProperty)) {
    return nativeScalarString(otherProperty.net ?? otherProperty.Net);
  }
  return '';
}
...
function addUnnamedWireNetLabels(
  ...
  const unnamedRoots = [...canonicalPointByRoot.entries()]
  ...
  for (const wire of wireItems) {
    ...
    for (let i = 1; i < points.length; i += 1) {
      ...
      if (previousPoint && currentPoint) {
        dsu.union(pointKey(previousPoint), pointKey(currentPoint));
      }
    }

    const netName = typeof wireSummary.net === 'string' ? wireSummary.net : '';
    const firstPoint = points[0];
    if (netName && firstPoint) {addNetLabel(firstPoint, netName);}
  }

  // Re-normalize root labels after all wire unions are known.
  for (const [root, names] of [...rootNetNames.entries()]) {
...
async function listNetsApi(budget?: NetDetailBudget): Promise<unknown> {
  ...
  for (const c of comps || []) {
    ...
    try {
      ...
      if (isNetDetailTimeout(error)) {throw error;}
      logRecoverableError('failed to inspect schematic component pins', error);
    }
  }
  ...
  if (schNetClass && typeof schNetClass.getAllNets === 'function') {
    try {
      ...
      if (isNetDetailTimeout(error)) {throw error;}
      logRecoverableError('failed to inspect schematic nets', error);
    }
  }
...
async function assignAutoDesignator(created: unknown): Promise<string | undefined> {
  ...
  try {
    ...
    logRecoverableError(`auto-designator: get(${pid}) failed`, error);
    return undefined;
  }
  if (!current) {return undefined;}

  const desig = nativeScalarString(safeGetState(current, 'Designator'));
  ...
  try {
    ...
    for (const c of comps || []) {
      ...
      if (rm) {
        const n = Math.trunc(Number(rm[1]));
        if (n > maxN) {maxN = n;}
      }
    }
  } catch (error) {
    logRecoverableError('auto-designator: scan failed', error);
  }
...
async function applyPlacedRotation(
  ...
  try {
    ...
    logRecoverableError(`apply-rotation: get(${pid}) failed`, error);
    return undefined;
  }
  if (!current) {return undefined;}
  const existingOther =
...
function readPinPoint(pin: unknown): Partial<PinPoint> {
  ...
  const point: Partial<PinPoint> = {};
  if (typeof x === 'number') {point.x = x;}
  if (typeof y === 'number') {point.y = y;}
  if (typeof rotation === 'number') {point.rotation = rotation;}
  return point;
}
...
async function resolvePinNoConnectState(
  ...
  const matches = (Array.isArray(pins) ? pins : []).filter(
    (pin) => readPinNumber(pin) === pinNumber,
  );
  if (matches.length === 0) {
    throw newBridgeError(
      'PIN_NOT_FOUND',
...
async function resolvePinEndpoint(
  ...
  const target = pinList.find((p) => readPinNumber(p) === pinNumber);
  if (!target) {
    throw newBridgeError(
      'EASYEDA_API_ERROR',
...
): Promise<{ primitiveId: string; designator: string; pinNumber: string }[]> {
  const floating: { primitiveId: string; designator: string; pinNumber: string }[] = [];
  try {
...
async function findFloatingPinsApi(): Promise<{
  floatingPins: { primitiveId: string; designator: string; pinNumber: string }[];
  partRefs: string[];
}> {
  const netlistData = (await listNetsApi()) as SchematicNetEntry[];
  const connectedNodes = buildConnectedNodeSet(netlistData);
  const floatingPins: { primitiveId: string; designator: string; pinNumber: string }[] = [];
...
async function getNetDetailApi(netName: string, operationTimeoutMs: unknown): Promise<unknown> {
  ...
  const allNets = (await listNetsApi(budget)) as { netName: string; nodes: unknown[] }[];
  ...
  switch (method) {
    case 'schematic.placeComponent': {
      ...
      try {
        ...
        logRecoverableError('auto-designator failed', error);
      }
      ...
      try {
        ...
        logRecoverableError('apply-rotation failed', error);
      }
      return createdComp;
    }
    case 'schematic.addWire': {
      const rawPoints: { x: number; y: number }[] = Array.isArray(params.points)
        ? params.points
        : [];
    ...
    case 'schematic.addPolygon': {
      ...
      return callFirst(
        ...
        (params.points as { x: number; y: number }[]).flatMap((p) => [p.x, p.y]),
        params.color ?? '#000000',
        params.fillColor ?? 'none',
        params.lineWidth ?? 1,
        params.lineType ?? 0,
    ...
    case 'schematic.connectPinsByNet': {
      const pins = params.pins as { primitiveId: string; pinNumber: string }[];
      const stubLength = typeof params.stubLength === 'number' ? params.stubLength : undefined;
      const netName = params.netName as string;
      const createdPrimitiveIds: string[] = [];
      const failures: { primitiveId: string; pinNumber: string; error: string }[] = [];
      for (const pin of pins || []) {
        try {
          const result = await connectPinToNetImpl(
            pin.primitiveId,
...
export function createDispatcher(toolkit: DispatcherToolkit): Dispatcher {
  ...
  schematicTransactionOperations = createSchematicTransactionOperations({
    ...
    deleteCachedTextAlignMode: (primitiveId) => {
  ...
  readOnlyOperations = createReadOnlyOperations({
    ...
    schematicInspection,
    listNets: async () =>
      (await listNetsApi()) as {
        netName: string;
        nodes: { component: string; pin: string }[];
      }[],
