...
function collectStatusGlobals(
  ...
  try {
    ...
    if (edaObject) {
      recordEdaDiagnostics(globals, edaObject, logRecoverableError);
    }
    if (EDAObject) {
      const boxedEDAObject = EDAObject;
...
export function createSystemApiOperations(
  ...
  return {
    ...
    async apiExecute(params) {
      ...
      const AsyncFunction = Object.getPrototypeOf(async () => null)
    ...
    async getStatus() {
      ...
      return {
        ...
        devMode: false,
        globals,
        hasEda: Boolean(edaObject) || Boolean(EDAObject),
