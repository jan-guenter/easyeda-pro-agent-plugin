...
function omitTrailingUndefined(values: unknown[]): unknown[] {
...
export function createCanvasOperations({
  ...
  async function capture(params: Record<string, unknown>): Promise<unknown> {
    const blob =
      typeof params.tabId === 'string'
        ? await callFirst(['DMT_EditorControl.getCurrentRenderedAreaImage'], params.tabId)
        : await callFirst(['DMT_EditorControl.getCurrentRenderedAreaImage']);
  ...
  async function captureRegion(params: Record<string, unknown>): Promise<unknown> {
    ...
    const blob = await callFirst(
      ['DMT_EditorControl.getCurrentRenderedAreaImage'],
      ...omitTrailingUndefined([params.tabId]),
    );
  ...
  async function locate(params: Record<string, unknown>): Promise<unknown> {
    return callFirst(
      ...
      ...omitTrailingUndefined([params.x, params.y, params.scaleRatio, params.tabId]),
    );
  }
