...
interface TextSnapshotRead {
  ...
  snapshot?: SchematicPrimitiveSnapshot | undefined;
}
...
function shiftWireLine(
  ...
  const flat = [...rawLine as number[]];
  for (let i = 0; i + 1 < flat.length; i += 2) {
    const x = flat[i];
    const y = flat[i + 1];
    if (x === undefined || y === undefined || !targetKeys.has(pointKey({ x, y }))) {continue;}
    flat[i] = x + dx;
    flat[i + 1] = y + dy;
    changed = true;
  }
...
function schematicPrimitiveClassPaths(kind: SchematicPrimitiveSnapshotKind): string[] {
  switch (kind) {
    ...
    default: {
      throw new TypeError(`Unsupported schematic primitive kind: ${String(kind)}`);
    }
  }
}
...
export function createSchematicTransactionOperations(
   ...
   */
  async function readPublicTextPrimitive(
    primitiveClass: any,
    path: string,
    primitiveId: string,
  ): Promise<unknown> {
    if (typeof primitiveClass.get !== 'function') {return null;}
    try {
  ...
  async function readPersistentTextPrimitive(
    ...
    if (typeof primitiveClass.getAll !== 'function') {return null;}
    try {
      const all = (await primitiveClass.getAll()) || [];
      if (!Array.isArray(all)) {return null;}
      return all.find((item) => extractPrimitiveId(item) === primitiveId) ?? null;
    } catch (error) {
  ...
  function componentSnapshotProperty(
    ...
    return compactDefinedRecord({
      ...
      otherProperty:
        (safeGetState(current, 'OtherProperty')) || {},
    });
  }
  ...
  async function listSchematicPrimitiveIds(
    ...
    if (values.length === 0 && typeof primitiveClass.getAllPrimitiveId === 'function') {
      try {
        const primitiveIds = (((await primitiveClass.getAllPrimitiveId()) || []) as unknown[])
          .filter(
            (value: unknown): value is string => typeof value === 'string' && value.length > 0,
          )
          .sort((a: string, b: string) => a.localeCompare(b));
        return { primitiveKind, primitiveIds: [...new Set(primitiveIds)] };
      } catch (error) {
  ...
  async function getComponentPinCoordinates(primitiveId: string): Promise<SchematicPoint[]> {
    ...
    try {
      ...
      logRecoverableError(`failed to read pins for ${primitiveId}`, error);
      return [];
    }
  ...
  async function followConnectedWires(
    ...
    try {
      ...
      logRecoverableError('failed to read wires while following a component move', error);
      return outcome;
    }
    ...
    for (const wire of wires) {
      ...
      try {
        ...
        logRecoverableError(`failed to follow wire ${wireId} after component move`, error);
