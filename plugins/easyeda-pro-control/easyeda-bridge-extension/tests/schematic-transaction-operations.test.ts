...
function createSubject(
  ...
  const operations = createSchematicTransactionOperations({
    ...
    readComponentType: (value) => {
      const componentType = readState(value, 'ComponentType');
      return typeof componentType === 'string' ? componentType.toLowerCase() : '';
    },
    readPinPoint: (value) => {
      const x = readState(value, 'X');
      const y = readState(value, 'Y');
      const rotation = readState(value, 'Rotation');
      return {
        ...(typeof x === 'number' ? { x } : {}),
        ...(typeof y === 'number' ? { y } : {}),
