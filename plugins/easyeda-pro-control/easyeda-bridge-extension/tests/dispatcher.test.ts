...
function primitiveIdOf(record: Record<string, unknown>): unknown {
  const getter = record.getState_PrimitiveId;
  return typeof getter === 'function' ? getter.call(record) : undefined;
}
...
function fakeNoConnectPin(
  ...
  pinNumber: string,
  initial: boolean,
  options?: { setter?: boolean; done?: boolean },
): Record<string, unknown> & { state: { noConnected: boolean } } {
  const resolvedOptions = { setter: true, done: true, ...options };
  ...
  if (resolvedOptions.setter) {
    pin.setState_NoConnected = (value: boolean) => {
      state.noConnected = value;
      return pin;
    };
  }
  if (resolvedOptions.done) {pin.done = vi.fn(async () => pin);}
  return pin;
}
...
describe('createDispatcher', () => {
  ...
  it('schematic.listNets uses a connected net port name instead of an anonymous name', async () => {
    ...
    const result = (await dispatcher.dispatch('schematic.listNets', {})) as {
      netName: string;
  ...
  it('system.apiInventory excludes runtime classes outside the API allowlist', async () => {
    ...
    })) as { classes: { className: string; methods: string[] }[]; total: number };
  ...
  it('api.call normalization preserves complete arrays, objects, state, and method metadata', async () => {
    ...
    const response = (await dispatcher.dispatch('api.call', {
      ...
      result: {
        ...
        repeatedReferences: Record<string, unknown>[];
      };
    };
  ...
  it('board.listLayers removes inactive catalogue placeholders', async () => {
    ...
    const result = (await dispatcher.dispatch('board.listLayers', {})) as {
      name: string;
      order: number;
    }[];
  ...
  describe('schematic primitive transaction snapshots', () => {
    ...
    it('captures a wire snapshot after component lookup misses', async () => {
      ...
      const dispatcher = createDispatcher(
        makeToolkit({
          SCH_PrimitiveComponent: { get: async () => {} },
  ...
  describe('schematic batch primitive support', () => {
    ...
    it('routes deletion to the primitive class that owns the ID', async () => {
      ...
      const dispatcher = createDispatcher(
        makeToolkit({
          SCH_PrimitiveComponent: { get: async () => {}, delete: componentDelete },
          SCH_PrimitiveText: {
            get: async (id: string) =>
      ...
      const dispatcher = createDispatcher(
        makeToolkit({
          SCH_PrimitiveWire: {
            create,
            getAll: async () => wires,
            get: async (id: string) => wires.find((wire) => primitiveIdOf(wire) === id),
          },
        }),
      );
      ...
      const dispatcher = createDispatcher(
        makeToolkit({
          SCH_PrimitiveText: {
            create,
            getAll: async () => texts,
            get: async (id: string) => texts.find((text) => primitiveIdOf(text) === id),
          },
        }),
      );
  ...
  describe('schematic.modifyPrimitive on text primitives', () => {
    it('preserves the public alignment instead of replaying getAll internal state', async () => {
      ...
      const textModify = vi.fn(
        async (_primitiveId: string, _property: Record<string, unknown>) => publicText,
      );
      const dispatcher = createDispatcher(
        makeToolkit({
      ...
      const nativeProperty = textModify.mock.calls[0]?.[1];
      if (nativeProperty === undefined) {
        throw new Error('Expected a native text modification payload.');
      }
