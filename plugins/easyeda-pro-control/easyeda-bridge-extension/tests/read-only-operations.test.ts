...
describe('read-only dispatcher operations', () => {
  ...
  it('rejects malformed required identifiers before calling host adapters', async () => {
    const dependencies = createDependencies();
    const operations = createReadOnlyOperations(dependencies as any);

    await expect(operations.getNetDetail({ netName: 42 })).rejects.toThrow(
      'netName must be a string',
    );
    await expect(operations.getPrimitiveSnapshot({ primitiveId: null })).rejects.toThrow(
      'primitiveId must be a string',
    );
    await expect(
      operations.getPinNoConnect({ primitiveId: 'cmp-1', pinNumber: false }),
    ).rejects.toThrow('pinNumber must be a string');
    expect(dependencies.getNetDetail).not.toHaveBeenCalled();
    expect(dependencies.schematicTransactionOperations.getPrimitiveSnapshot).not.toHaveBeenCalled();
    expect(dependencies.getPinNoConnect).not.toHaveBeenCalled();
  });
