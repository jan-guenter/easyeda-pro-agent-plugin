...
function missingRuntimePath<T>(_paths: readonly string[]): T | undefined {
  return new Map<string, T>().get('missing');
}
...
function createDependencies(overrides: Record<string, unknown> = {}) {
  return {
    ...
    buildId: 'build-test',
    inspectApiInventory: vi.fn((filter?: string) =>
      filter === undefined ? {} : { filter },
    ),
    callAllowedApi: vi.fn(async (path: string, args: unknown[]) => ({ path, args })),
    readFirstPath: missingRuntimePath,
...
describe('system API operation fallbacks', () => {
  ...
  it('fails component inspection for missing APIs and handles non-array native results', async () => {
    const missingDependencies = createDependencies({ readFirstPath: vi.fn(() => {}) });
    await expect(
