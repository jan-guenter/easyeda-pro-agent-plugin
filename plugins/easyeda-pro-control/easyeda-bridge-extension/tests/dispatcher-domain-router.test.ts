...
import type { SchematicPrimitiveSnapshot } from '../src/schematic-transaction-operations.js';

const ROUTER_SNAPSHOT: SchematicPrimitiveSnapshot = {
  schemaVersion: 'schematic-primitive-snapshot/v1',
  primitiveId: 'wire-1',
  primitiveKind: 'wire',
  property: { line: [0, 0, 1, 1], net: 'GND' },
};
const DRC_RESULT = {
  violations: [],
  totalViolations: 0,
...
function createDependencies() {
  return {
    projectOperations: {
      ...
      save: vi.fn(async (_params: Record<string, unknown>) => {}),
    ...
    boardInspection: {
      requireActivePcbContext: vi.fn(async () => {}),
    ...
    designRuleCheckOperations: {
      runDrc: vi.fn(async () => DRC_RESULT),
      runErc: vi.fn(async () => ERC_RESULT),
      runRuleCheck: vi.fn(async () => DRC_RESULT),
      runSchematicCheck: vi.fn(async () => DRC_RESULT),
    },
    canvasOperations: {
    ...
    pcbMutationOperations: {
      ...
      deleteComponents: vi.fn(async (_params: Record<string, unknown>) => ({
        success: true,
        deletedCount: 0,
        deleted: [],
        notFound: [],
    ...
    schematicTransactionOperations: {
      deletePrimitives: vi.fn(async (primitiveIds: string[]) => ({
        success: true,
        deleted: primitiveIds,
        notFound: [],
      })),
      recreatePrimitiveSnapshot: vi.fn(async (_snapshot: unknown) => ({
        primitiveId: ROUTER_SNAPSHOT.primitiveId,
        snapshot: ROUTER_SNAPSHOT,
      })),
      restorePrimitiveSnapshot: vi.fn(async (_snapshot: unknown) => ({
      ...
      getPrimitiveSnapshot: vi.fn(async () => ROUTER_SNAPSHOT),
      listPrimitiveIds: vi.fn(async () => ({ primitiveKind: 'wire' as const, primitiveIds: [] })),
    },
    readOnlyOperations: {
...
describe('createDispatcherDomainRouter', () => {
  ...
  it('delegates exact methods without changing parameters or results', async () => {
    ...
    await expect(router.tryDispatch('pcb.deleteComponent', params)).resolves.toMatchObject({
      handled: true,
      value: { success: true, deletedCount: 0, deleted: [], notFound: [] },
    });
    ...
    await expect(
      ...
      handled: true,
      value: { success: true, deleted: ['wire-1'], notFound: [] },
    });
    const snapshot = ROUTER_SNAPSHOT;
    await expect(
      router.tryDispatch('schematic.recreatePrimitiveSnapshot', { snapshot }),
    ).resolves.toEqual({
      handled: true,
      value: { primitiveId: 'wire-1', snapshot },
    });
    await expect(
      router.tryDispatch('schematic.restorePrimitiveSnapshot', { snapshot }),
    ).resolves.toEqual({
      handled: true,
      value: { restored: true, snapshot },
    });
    await expect(
