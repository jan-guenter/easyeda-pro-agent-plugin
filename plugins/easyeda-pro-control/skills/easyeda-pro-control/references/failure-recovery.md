...
2. Obtain a fresh user message after the authorized restart containing the current `runtimeRestartChallenge` shown by `easyeda_control_recover_incomplete`, and pass that exact value as `runtimeRestartConfirmation`. Its form is `EASYEDA_RESTARTED_AND_RECONNECTED:<operationId>:<phase>:<attempt>:<nonce>`. It is operation-, phase-, attempt-, and nonce-bound and is consumed durably before the orphan gate opens; an interrupted consumption causes a new nonce, and an old value cannot be replayed. Never synthesize it, copy it from an error, infer it from a reconnect, or reuse an earlier value. The facade verifies the reviewed runtime fingerprint after reconnect but cannot independently prove process generation or which choice was made in a native prompt.
...
A bridge disconnect during a read invalidates that read. A disconnect during a write makes the result unknown. If the journal marks an orphan risk, use the destructive human restart gate above before reconnecting through the single facade-owned child; merely reconnecting can leave the delayed call alive. Do not start a competing server.
