...
function assertUniqueNames(
  rows: readonly ValidationRecord[],
  label: string,
): void {
...
function assertPayloadCandidate(
  value: unknown,
): asserts value is ExactPayloadCandidate {
...
const selectorSchema = z
  ...
  .superRefine((value, context) => {
    const choices = [
      value.all === true,
      value.primitiveIds !== undefined,
      value.designators !== undefined,
    ];
    if (choices.filter(Boolean).length !== 1) {
      context.addIssue({
        code: "custom",
        message:
          "selector requires exactly one of all=true, primitiveIds, or designators.",
      });
    }
...
function assertPadShape(value: unknown, label: string): void {
  ...
  if (
    typeof shape === "string" &&
    ["ELLIPSE", "OVAL", "NGON"].includes(shape)
  ) {
    if (value.length !== 3) {
...
function assertHeatWelding(value: unknown, label: string): void {
  ...
  if (
    ...
    !["Divergent", "Direct-connected", "Non-connected"].includes(
...
export function validateExactReadPayload<T>(
  payload: T,
  ...
  if (
    request.kind === "schematic-components" ||
    request.kind === "pcb-components"
  ) {
    if (
    ...
    for (const id of keys) {
      ...
      if (
        component.designator !== null &&
        typeof component.designator !== "string"
      ) {
        throw new Error(
      ...
      if (schematic) {
        ...
        if (request.includePins) {
          ...
          for (const [index, pin] of component.pins.entries()) {
            ...
            if (
              typeof pin.pinShape !== "string" ||
              typeof pin.pinType !== "string"
            ) {
        ...
        if (
          typeof component.primitiveLock !== "boolean" ||
          typeof component.addIntoBom !== "boolean"
        ) {
          throw new TypeError(
        ...
        if (request.includePins) {
          ...
          for (const [index, pad] of component.pads.entries()) {
            ...
            if (
              pad.primitiveType !== "ComponentPad" ||
              pad.parentComponentPrimitiveId !== id
            ) {
            ...
            if (
              pad.source !== "component-pin-wrapper-transformed-placement-only"
            ) {
    ...
    for (const component of payload.compiledConnectivity) {
      ...
      if (
        ...
        JSON.stringify(compiledPinNumbers) !==
          JSON.stringify([...compiledPinNumbers].toSorted())
      ) {
        throw new Error(
      ...
      const correlation =
        payload.componentCorrelation.byUniqueId[component.uniqueId];
      assertExactRecordKeys(
        correlation,
      ...
      if (
        ...
        JSON.stringify(correlation.pinNumbers) !==
          JSON.stringify(compiledPinNumbers)
      ) {
        throw new Error(
    ...
    if (
      ...
      typeof payload.componentPadCorrelation.byComponentPrimitiveId !==
        "object" ||
    ...
    const monitoredFamilies = new Set<PcbInventoryFamilyName>(
      PCB_MONITORED_FAMILY_NAMES,
    );
    const validatedFamilies = new Map<
      PcbInventoryFamilyName,
      ValidatedFamily
    >();
    ...
    for (const name of PCB_INVENTORY_FAMILY_NAMES) {
      ...
      if (familyIndex !== undefined) {
        ...
        const indexEntries: [string, ValidationRecord][] = [];
      ...
      validatedFamilies.set(name, {
        ...
        ...(validatedFamilyIndex === undefined
          ? {}
          : { byPrimitiveId: validatedFamilyIndex }),
      });
    }
    if (
    ...
    const getMonitoredFamily = (
      name: PcbMonitoredFamilyName,
    ): ValidatedMonitoredFamily => {
    ...
    const validatePrimitiveRecord = (
      ...
      if (
        ["pads", "lines", "arcs", "polylines", "pours", "fills"].includes(name)
      ) {
      ...
      if (name === "pads") {
        ...
        if (
          typeof row.padNumber !== "string" ||
          !Number.isInteger(row.padType)
        ) {
        ...
        if (
          typeof row.preserveSilos !== "boolean" ||
          typeof row.primitiveLock !== "boolean"
        ) {
    ...
    const padIdsByComponentPrimitiveId =
      componentPadCorrelation.byComponentPrimitiveId;
    ...
    if (
      ...
      payload.standalonePadCount !==
        padsFamily.count - correlatedPadIds.length ||
    ...
    if (
      ...
      JSON.stringify([...mappedPadIds].toSorted()) !==
        JSON.stringify(correlatedPadIds)
    ) {
      throw new Error(
    ...
    for (const id of correlatedPadIds) {
      ...
      const ownedPadIds =
        padIdsByComponentPrimitiveId[correlation.parentComponentPrimitiveId];
      if (
        directPad.parentComponentPrimitiveId !==
    ...
    for (const id of padsFamily.primitiveIds) {
      ...
      if (
        !directPad ||
        isCorrelated !==
          Object.hasOwn(directPad, "parentComponentPrimitiveId") ||
    ...
    for (const id of pouredParentIds) {
      ...
      if (
        row.primitiveId !== id ||
        row.pourPrimitiveId !== id ||
        !Array.isArray(row.pourFills)
      ) {
        throw new Error(
      ...
      if (
        ...
        JSON.stringify(rowFillIds) !==
          JSON.stringify([...rowFillIds].toSorted())
      ) {
        throw new Error(
    ...
    const groupedRuleRows: readonly (readonly [string, ValidationRecord[]])[] =
      [
    ...
    for (const group of rules.padPairGroups) {
      if (
        !Array.isArray(group.padPairs) ||
        !Array.isArray(group.minimumWireLengths)
      ) {
        throw new TypeError(
      ...
      const lengthKeys = group.minimumWireLengths.map((row) =>
        JSON.stringify(row?.padPair),
      );
      if (
        group.padPairs.some(
          (pair) =>
        ...
        group.minimumWireLengths.some(
          (row) =>
            typeof row?.minWireLength !== "number" ||
            !Number.isFinite(row.minWireLength),
        ) ||
