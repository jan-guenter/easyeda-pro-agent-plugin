...
function upstreamEnvironment(): Record<string, string> {
  ...
  return {
    ...
    DATA_DIR:
      process.env["EASYEDA_UPSTREAM_DATA_DIR"] ?? `${controlRoot}/upstream`,
  };
}
...
async function treeSha256(
  root: string,
): Promise<ImplementationTreeFingerprint> {
  const files: string[] = [];
...
function upstreamCommand(): string {
  return (
    process.env["EASYEDA_UPSTREAM_COMMAND"] ??
    "/root/.nvm/versions/node/v24.18.0/bin/npx"
  );
}

async function dependencyLockFingerprint(
  cwd: string,
): Promise<DependencyLockFingerprint> {
...
function stableValue(value: unknown): unknown {
  ...
  if (Array.isArray(value)) {
    return value.map((item: unknown) => stableValue(item));
  }
  if (!isRecord(value)) {
    return value;
  }
...
async function closeIgnoringFailure(resource: {
  ...
  try {
    ...
    // Cleanup failure must not mask the connection or shutdown failure already in progress.
  }
}
...
export class UpstreamEasyedaClient {
  ...
  private stderr = "";
  private startupLauncherFingerprint: LauncherFingerprint | null = null;
  ...
  async #connect(): Promise<Client> {
    ...
    // Capture the implementation before spawning the child.
    // A later fingerprint can attribute on-disk edits to the already running child.
    ...
    const serverParameters: StdioServerParameters = {
      ...
      stderr: "pipe",
      ...(upstreamCwd !== undefined && upstreamCwd.length > 0
        ? { cwd: upstreamCwd }
        : {}),
    };
    ...
    try {
      ...
      await closeIgnoringFailure(transport);
  ...
  public async launcherFingerprint(): Promise<LauncherFingerprint> {
    const command = upstreamCommand();
