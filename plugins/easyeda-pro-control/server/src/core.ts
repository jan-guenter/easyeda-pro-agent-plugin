...
export function classifyTool(tool: unknown): ToolClassification {
  ...
  const knownWriteName =
    !pinnedReadException &&
    /(^|_)(add|apply|begin_transaction|commit|create|delete|export|import|modify|move|place|recover|rollback|route|save|set|sync|update|write)(_|$)/iu.test(name);
...
export function validateRawExecutionInput(input: unknown) {
  if (!isRecord(input)) throw new Error('Raw execution input must be an object.');
  ...
  if (!['read', 'mutate-unsaved', 'persist', 'native-ui'].includes(String(input['mode']))) {
...
export function validateEvidencePaths(evidence?: unknown): EvidencePaths | undefined {
...
function fingerprintRequirementMissing(value: unknown, kind: FingerprintRequirementKind): boolean {
...
export function assertExpectedFingerprint(
  ...
  const missing: Array<{ pointer: string; required: string }> =
...
function assertFacadeManifest(
  ...
  const fileCount = manifestPositiveInteger(record['fileCount'], `${pointer}/fileCount`);
  ...
  files.forEach((file, index) => {
    ...
    const fileRecord = manifestObject(file, filePointer, ['relativePath', 'bytes', 'sha256']);
  ...
  if (
    ...
    canonicalJson(relativePaths) !== canonicalJson([...relativePaths].toSorted())
  ) {
    throw new Error(
...
function assertReviewedCompatibilityManifest(
  ...
  const toolCatalog = manifestObject(upstream['toolCatalog'], '/upstream/toolCatalog', [
    'count',
    'sha256',
  ]);
...
export function evaluateAssertions(
  ...
  return assertions.map((assertion, index) => {
    ...
    switch (assertion.op) {
      ...
      case 'matches':
        passed =
          typeof actual === 'string' && new RegExp(stringValue(assertion.value), 'u').test(actual);
        break;
      case 'length-equals':
...
export function assertSubset(actual: unknown, expected: unknown, label = 'value'): true {
  const mismatches = compareSubset(actual, expected);
  if (mismatches.length > 0) {
...
export function validateCallSource(spec: unknown): void {
  if (!isRecord(spec) || spec['toolName'] !== 'easyeda_execute') return;
  const argumentsRecord = isRecord(spec['arguments']) ? spec['arguments'] : undefined;
  ...
  const timeoutMs = Number(argumentsRecord?.['timeoutMs'] ?? 15000);
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1000 || timeoutMs > 60000) {
...
function checkpointPointer(checkpoint: unknown) {
...
function summarizeArtifact(artifact: unknown) {
...
export function operationSummary(operation: Readonly<UnknownRecord>) {
  const artifacts = Array.isArray(operation['artifacts']) ? operation['artifacts'] : [];
  const lastError = isRecord(operation['lastError']) ? operation['lastError'] : undefined;
  return {
    ...
    artifacts: {
      ...
      recent: artifacts.slice(-12).map((artifact) => summarizeArtifact(artifact)),
    },
