...
void describe('upstream result normalization and tool classification', () => {
  ...
  void test('payload extraction unwraps bounded result envelopes and parses text fallback', () => {
    ...
    assert.deepEqual(
      extractToolPayload({ content: [{ type: 'text', text: '{"documentType":3}' }] }),
      { documentType: 3 },
    );
    assert.deepEqual(
  ...
  void test('classification is conservative when schemas or names imply a write', () => {
    assert.deepEqual(
      ...
      { readOnly: true, write: false, hasConfirmWrite: false, idempotent: true },
    );
...
void describe('raw execution and evidence input guards', () => {
  ...
  void test('raw execution accepts exactly one source and the hard timeout boundary', () => {
    ...
    assert.deepEqual(
      validateRawExecutionInput({ ...validRawInput, code: undefined, scriptPath: '/tmp/a.js', timeoutMs: 1000 }),
  ...
  void test('normalizes POSIX, Windows, and file URI project paths to the same database identity', () => {
    ...
    assert.equal(
      normalizeEasyedaProjectPath('C:\\Users\\Fixture\\Board Demo.eprj2'),
...
void describe('JSON pointers and postcondition assertions', () => {
  ...
  void test('subset comparison reports precise escaped pointers and assertion details', () => {
    ...
    assert.deepEqual(compareSubset(actual, { context: { project: { uuid: 'project-1' } } }), []);
    assert.deepEqual(compareSubset(actual, { 'path/key': { value: 4 } }), [
      { pointer: '/path~1key/value', expected: 4, actual: 3 },
    ]);
    assert.equal(assertSubset(actual, { context: { project: { uuid: 'project-1' } } }), true);
...
void describe('pinned raw call specifications', () => {
  ...
  const valid = {
    ...
    arguments: { code, confirmWrite: true, timeoutMs: 60000 },
  ...
  void test('requires exact source hash, bounded timeout, confirmation, and safe operation mode', () => {
    ...
    const invalid = [
      ...
      { ...valid, arguments: { ...valid.arguments, timeoutMs: 60001 } },
  ...
  const valid = {
    ...
    installedBundles: {
      ...
      pcbEditor: {
        ...
        implementationPath: '/opt/easyeda/assets/pro-pcb/pcb.js',
        implementationSha256: '65401cdc0a8f244db2ff2d8da88fd835b6e1fb3a3ecdbcfd975781502cb04b54',
      },
      publicApi: {
        version: '0.2.53.aee2f57a',
        implementationPath: '/opt/easyeda/assets/pro-api/api.js',
        implementationSha256: '5923696711fc5e4f3027ce500d5ba6aee57b9d8f9903fdba84820432066125fc',
        adapterPath: '/opt/easyeda/assets/pro-api/adapter.js',
        adapterSha256: '4da5b5184a78e2d3aca843dad6b147d7feb7ec1368160d73f49c4acbcf97dfdb',
        declarationsPath: '/opt/easyeda/assets/pro-api/api-types.d.ts',
        declarationsSha256: '32a0d2f8b4bc3d7b2b93b33499d9d768b0c23c77f45843a65166cf4e8ad6dab1',
      },
    },
...
void describe('plan identity and operation summaries', () => {
  void test('plan hashes are key-order independent, but cover every execution-bearing field', () => {
    const base = {
      ...
      reopenedVerifyCalls: [],
      reopenedAssertions: [],
      checkpoint: { source: '/tmp/project.eprj2', outputDir: '/tmp/checkpoints', label: 'pre' },
    };
    const reordered = {
      checkpoint: { label: 'pre', outputDir: '/tmp/checkpoints', source: '/tmp/project.eprj2' },
    ...
    assert.notEqual(
      buildPlanHash(base),
      buildPlanHash({ ...base, applyCall: { ...base.applyCall, args: { ref: 'R1', x: 9, y: 2 } } }),
    );
  ...
  void test('operation summaries expose bounded diagnostics and managed evidence pointers', () => {
    ...
    const summary = operationSummary({
      ...
      lastError: { name: 'E'.repeat(200), message: 'm'.repeat(3000), stack: 'not included' },
    ...
    assert.equal(summary.journalPath, '/control/operations/easyeda-operation-1.json');
  ...
  void test('orphaned-call risk is explicit while remaining conservative for legacy journals', () => {
    ...
    assert.equal(
      operationHasOrphanedCallRisk({ state: 'unknown', orphanedCallPossible: false }),
      false,
    );
    const summary = operationSummary({
